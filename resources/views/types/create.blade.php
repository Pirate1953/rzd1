  ...
  <body class="bg-light">
    ...
    <div class="container">
      <div class="py-5 text-center">
        ...
        <h2>{{__('Add type')}}</h2>
        <p class="lead" style="color: black !important;">{{__('Эта форма используется для добавления типов билетов. Типы билетов также как и зоны будут определять цену билета.')}}</p>
      </div>
    ...
    {{-- Форма предъявляется методом HTTP POST на маршрут types.store --}}
    {{
        Form::model($type, [
            'method' => 'POST',
            'route'  => 'types.store'
